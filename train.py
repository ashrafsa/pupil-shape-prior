...
def main():
    ...
    parsed_image_dataset = raw_image_dataset.map(parameters._parse_image_function).shuffle(
        buffer_size=int(shuffle_buffer / 10)).batch(
    ...
    model = Model_UNet_Segmentation.Unet4f32chSigmoid()

    print(parameters.C_GREEN + nf_nch + ', ' + str(cross_val_num).zfill(3) + ' : ' + str(
        image_num) + ' images' + parameters.C_END)
    ...
    for image_features in parsed_image_dataset:
        ...
        for bat in range(bat_len):
            if np.count_nonzero(output_bk[bat, 0, :, :]) != 0:
                ...
                cv2.line(temp_mat, (center_pt[0] + 100, center_pt[1] + 241), (center_pt[0] - 100, center_pt[1] - 241),
                         255)
                cv2.line(temp_mat, (center_pt[0] + 100, center_pt[1] - 241), (center_pt[0] - 100, center_pt[1] + 241),
                         255)
                cv2.line(temp_mat, (center_pt[0] + 241, center_pt[1] + 100), (center_pt[0] - 241, center_pt[1] - 100),
                         255)
                cv2.line(temp_mat, (center_pt[0] + 241, center_pt[1] - 100), (center_pt[0] - 241, center_pt[1] + 100),
                         255)
                ...
                if np.sum(edges) != 0:
                    ...
                    edge_loca = edge_loca[:min(8, len(edge_loca[:, 0])),
                                :]  # extracting random 8 point on boundary points
                    ...
                    for edge_ind in range(len(edge_loca[:, 0])):
                        ...
                        if np.sum(line_mat) != 0:
                            ...
                            for [line_x, line_y] in line_loca:
                                if label_2ch[bat, 0, line_y, line_x] == label_2ch[bat, 0, edge_y_0, edge_x_0] == \
                                        label_2ch[bat, 0, edge_y_1, edge_x_1]:
                                    loss_t2 += torch.abs(
                                        label_2ch[bat, 0, edge_y_0, edge_x_0] - output[bat, 0, edge_y_0, edge_x_0]) \
                                               * torch.abs(
                                        label_2ch[bat, 0, edge_y_1, edge_x_1] - output[bat, 0, edge_y_1, edge_x_1]) \
                                               * torch.abs(
                                        output[bat, 0, edge_y_0, edge_x_0] + output[bat, 0, edge_y_1, edge_x_1] - 2 *
                                        output[bat, 0, line_y, line_x])
                                    num2 += 1
                        if flag == True:
                            break
        #########################################
        ...
        if iter == 12:
            ...
            common.print_expected_time_train(total_batch=total_batch, total_epoch=parameters.total_epoch,
                                             start_time=start_time, cost=cost)
        ...
        if current_batch == total_batch:
            ...
            print(str(cross_val_num).zfill(
                3) + ' Time : {:02}:{:02}:{:02}\tEpoch: {:03}\tIters: {:06}\tTraining Loss: {:.6f}\t lr: {:.6f}'.format(
